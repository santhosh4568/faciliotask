<template>
  ...
  <el-aside width="200px" style="background-color:white">
    <div v-for="customer in customers" :key="customer.id">
     <router-link :to="{name:'CustomerDetails',params:{id:customer.id}}" class="link"  >
  ...
  <el-container>
    <el-header style="display:flex ; flex-direction:column; justify-content:flex-start;" >
      <h5 style="color:rgb(57,178,194)">#{{customer.id}}</h5>
      <h4 style="color:rgb(50, 64, 86);font-weight:400;font-size:22px;padding-top:10px;">{{customer.name}}</h4>
      <el-tabs v-model="activeName">
      <el-tab-pane  label="CUSTOMER" name="first">
        <div class="sumcard">
          <h3>Name:<span> {{customer.name}}</span></h3>
          <h3>Address:<span>{{customer.address}}</span></h3>
          <h3>City:<span>{{customer.city}}</span></h3>
          <h3>Contact:<span>{{customer.contact}}</span></h3>
        </div>
      </el-tab-pane>
